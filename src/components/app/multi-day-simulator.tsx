...
// --- Child Components for Refactoring ---

interface SimulatorFormProps {
    form: UseFormReturn<MultiDayFormValues>;
    onSubmit: SubmitHandler<MultiDayFormValues>;
    isLoading: boolean;
    stopSimulation: () => void;
    lastRunData: MultiDayFormValues | null;
    handleSaveRun: () => void;
}

const SimulatorForm: FC<SimulatorFormProps> = ({ form, onSubmit, isLoading, stopSimulation, lastRunData, handleSaveRun }) => (
...
interface SimulationOutputProps {
    isLoading: boolean;
    results: TimeIntervalResult[] | null;
    progress: number;
    form: UseFormReturn<MultiDayFormValues>;
    visibleSeries: SeriesVisibility;
    toggleSeries: (datakey: keyof SeriesVisibility) => void;
    dailyTotals: any[];
}

const SimulationOutput: FC<SimulationOutputProps> = ({ isLoading, results, progress, form, visibleSeries, toggleSeries, dailyTotals }) => {
    return (
          ...
          <div className="flex flex-col gap-8 animate-in fade-in duration-500">
             {isLoading && (
               <div className="w-full max-w-md p-4 text-center mx-auto">
                  <p className="text-lg font-semibold mb-2">Running {form.getValues('numDays')}-day simulation... ({progress.toFixed(0)}%)</p>                  <Progress value={progress} className="w-full" />
                </div>
            )}
            <Card>
                <CardHeader>
...
interface SavedRunsTableProps {
    leaderboard: MultiDayLeaderboardEntry[];
    onSelect: (entry: MultiDayLeaderboardEntry) => void;
    onDelete: (id: string) => void;
}

const SavedRunsTable: FC<SavedRunsTableProps> = ({ leaderboard, onSelect, onDelete }) => (
    <Card>
        <CardHeader>
...
export default function MultiDaySimulator() {
  ...
  useEffect(() => {
    if (selectedEntry) {
      form.reset({
        ...
        basePCVR: selectedEntry.basePCVR * 100,
        overallError: selectedEntry.overallError * 100,
        dayVolatility: selectedEntry.dayVolatility * 100,
        volatility: selectedEntry.volatility * 100,
  ...
  const runAndProcessSimulation: SubmitHandler<MultiDayFormValues> = async (data) => {
    ...
    setTimeout(async () => {
      const simulationParams = {
        ...data,
        basePCVR: data.basePCVR / 100,
        overallError: data.overallError / 100,
  ...
  const handleSaveRun = () => {
    ...
    const newEntry: MultiDayLeaderboardEntry = {
        ...
        finalBudgetUtilisation: finalInterval.dayBudgetUtilisation,
        ...lastRunData,
        results: results,
  ...
  return (
    <div className="flex flex-col gap-8">
        <SimulatorForm
            form={form}
            onSubmit={runAndProcessSimulation}
            isLoading={isLoading}
